...
cp .maestro/state.local.md .maestro/checkpoints/{name}/state.local.md
cp -r .maestro/memory/ .maestro/checkpoints/{name}/memory/
```

The `state.local.md` copy inside the checkpoint directory ensures that when restoring, both the canonical state file and the named snapshot are in sync.
...
3. Restore state: `cp .maestro/checkpoints/{name}/state.local.md .maestro/state.local.md`
4. Restore memory: `cp -r .maestro/checkpoints/{name}/memory/ .maestro/memory/`

The `state.local.md` is restored alongside the code, ensuring state and code are always in sync after a revert.
5. Confirm:
